...
namespace Invector.vCharacterController
{
    ...
    public class vHeadTrack : vMonoBehaviour
    {
        ...
        [vMinMax(minLimit = -180f, maxLimit = 180f)] public Vector2 horizontalAngleLimit = new Vector2(-90, 90);
        [vMinMax(minLimit = -90f, maxLimit = 90f)] public Vector2 verticalAngleLimit = new Vector2(-90, 90);     
        ...
        public virtual void SetLookAtPosition(Vector3 point, float headWeight, float spineWeight)
        {
            ...
            xAngle = Mathf.Clamp(Mathf.Lerp(xAngle, (x), smooth * Time.fixedDeltaTime), verticalAngleLimit.x, verticalAngleLimit.y);
            yAngle = Mathf.Clamp(Mathf.Lerp(yAngle, (y), smooth * Time.fixedDeltaTime), horizontalAngleLimit.x, horizontalAngleLimit.y);

            var xSpine = NormalizeAngle(xAngle + Quaternion.Euler(offsetSpine).eulerAngles.x);
            var ySpine = NormalizeAngle(yAngle + Quaternion.Euler(offsetSpine).eulerAngles.y);

            foreach (Transform segment in spine)
            {
                var rotX = Quaternion.AngleAxis((xSpine * spineWeight) / spine.Count, segment.InverseTransformDirection(transform.right));
            ...
            if(head)
            {
                var xHead = NormalizeAngle(xAngle - (xSpine * spineWeight) + Quaternion.Euler(offsetHead).eulerAngles.x);
                var yHead = NormalizeAngle(yAngle - (ySpine * spineWeight) + Quaternion.Euler(offsetHead).eulerAngles.y);
                var _rotX = Quaternion.AngleAxis(xHead * headWeight, head.InverseTransformDirection(transform.right));
                var _rotY = Quaternion.AngleAxis(yHead * headWeight, head.InverseTransformDirection(transform.up));
                head.rotation *= _rotX * _rotY;// *Quaternion.Euler(offsetHead);
            }
