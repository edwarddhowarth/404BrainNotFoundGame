...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonInput : vMonoBehaviour
    {
        ...
        [vHelpBox("Character will rotate to the Camera Forward while walking when Strafing")]
        public bool rotateToCameraFwdWhenMoving = true;
        [vHelpBox("Character will rotate to the Camera Forward at all times when Strafing")]
       ...
       /// </summary>
        ...
        {
            get { if (cc) return cc.keepDirection; return false; }
            set { if (cc) cc.keepDirection = value; }
        }
        ...
        {   
            if(!cameraMain)
            {
                ...
                else cameraMain = Camera.main;
            }
           
            if (cameraMain && !ignoreCameraRotation)
            {
                if (!keepDirection) cc.UpdateTargetDirection(cameraMain.transform);
        ...
        {
            if (rotateToCameraFwdWhenMoving && cc.isStrafing && !cc.actions && !cc.lockMovement)
            {
